...
cld %>% pre_print %>% ggplot(aes(children, f)) + geom_col(fill = "white", color = "black")
```
...
```{r }
wages %>% hist_table(size, cols = "dmMfnFp", breaks = c(20, 100, 250),
                     total = TRUE)
```
...
```{r }
wages %>% hist_table(size, vals = "xlua", breaks = c(20, 100, 250), wlast = 2)
```
