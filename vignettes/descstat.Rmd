...
`pre_plot` returns an `ypos` series which indicate the coordinate
...
- a specific value for the last class can be indicated using the
  `xlast` argument,
- the width of the last class (if it is open on infinity on the right)
...
class is closed on the left (`a` is included) and open on the right
(`b` is not included) as for the second case, the class is open on the
left and closed on the right. This is the notation used when the
...
The `cls2val` function takes as arguments a class series, a `pos`
argument and the three arguments `xfirst`, `xlast` and `inflate`
previously described. `pos` is a numerical value between 0 and 1:
...
`geom_polygon` for an histogram and to `geom_line` for a frequency
polygon.
...
`hist_table` objects. The problem is that only a few of `base` or
`stats` package of **R** functions are generic. For these, methods
where written for `hist_table` objects, for the other ones, we had to
...
To get a "wide" table, with the values of one of the two variable
...
method replace classes by values.
