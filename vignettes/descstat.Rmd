...
- merging high values of integer series in a `>=N` cathegory,
- computing cummulative distributions, 

are not tedious. Moreover, to our knowledge, **R** offer weak support
...
`plot` argument equal to `banner` or `cummulative`. With `plot =
"banner"`,
...
```{r }
wages %>% hist_table(size, cols = "dMfnFp", breaks = c(20, 100, 250),
                     total = TRUE)
```
...
```{r }
wages %>% hist_table(size, breaks = c(20, 100, 250), wlast = 2)
```
...
```{r }
wages %>% hist_table(size, vals = "xlua", cols = "p", breaks = c(20, 100, 250), wlast = 2)
```
...
```{r }
lzc <- wages %>% hist_table(wage, "MF", breaks = c(10, 20, 30, 40, 50)) %>%
    pre_plot(plot = "lorenz")
lzc
```
Each line in the resulting tibble indicate the coordonates (`F` for
...
these, methods where written for `hist_table` objects, for the other
...
- the conditionnal distribution is obtained by dividing the joint by
